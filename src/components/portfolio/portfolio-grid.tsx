...
export function PortfolioGrid({ items, locale = 'cs' }: PortfolioGridProps) {
    ...
    // Filter items by orientation/category first
    const orientationItems = items.filter((item) => {
        if (orientation === "ugc") {
            return item.category === "UGC";
        }
        // For horizontal/vertical, exclude UGC items
        const itemOrientation = item.orientation || "horizontal";
        return itemOrientation === orientation && item.category !== "UGC";
    });
    ...
    const labels = {
        ...
        ugc: locale === 'en' ? '🎭 UGC & Acting' : '🎭 UGC Videa',
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Grid */}
            <div key={orientation} className={cn("grid gap-6", orientation === "vertical" || orientation === "ugc" ? "grid-cols-2 md:grid-cols-3 lg:grid-cols-4" : "md:grid-cols-2 lg:grid-cols-3")}>
